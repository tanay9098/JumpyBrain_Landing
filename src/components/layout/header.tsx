    ...
    <header className="sticky top-0 z-50 border-b border-border bg-bg/80 backdrop-blur-md">
      <div className="mx-auto flex h-18 w-full max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
        ...
        >
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-full px-3 py-2 text-sm font-medium text-text-secondary transition-colors hover:bg-surface-active hover:text-text-primary"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <div className="hidden min-[1160px]:inline-flex">
            <ButtonLink
